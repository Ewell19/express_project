...
if [ ! -f sprint.txt ]; then
  ...
  # check if text output matches valid sprint format: 12, 13, 15, 12-ft, 13-ft, or 15-ft
  if [[ ! $TEXT_OUTPUT =~ ^(12|13|15)(-ft)?$ ]]; then
    print_red "$TEXT_OUTPUT in file 'sprint.txt' is not a valid sprint number!"
    print_orange "Valid options are: 12, 13, 15, 12-ft, 13-ft, or 15-ft"
    exit 1
  else
...
# Extract base sprint number (12, 13, or 15) for file checking
BASE_SPRINT=$(echo $SPRINT_NUMBER | grep -o '^[0-9]\+')

# Only auto-upgrade sprint 12 projects to sprint 13 based on auth/config files
if [[ $BASE_SPRINT == 12 ]]; then
...
if [[ ! $SPRINT_NUMBER =~ ^(12|13|15)(-ft)?$ ]]; then
